...
export default function ShareButton({ score, className = "", label }: ShareButtonProps) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);

  const message = score
    ? `I scored ${score.correct}/${score.total} on MPSC PYQ QUIZ! Try it free:`
    : SHARE_TEXT;

  const fullLink = `${message}\n${SITE_URL}`;
  const encodedText = encodeURIComponent(fullLink);
  ...
  const nativeShare = () => {
    if (navigator.share) {
      navigator.share({ title: "MPSC PYQ QUIZ", text: message, url: SITE_URL }).catch(() => {});
    } else {
      setOpen((v) => !v);
    }
  };
            ...
            >
              {copied ? (
                ...
                <svg className="h-5 w-5 text-violet-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-3.061a4.5 4.5 0 00-1.242-7.244l-4.5-4.5a4.5 4.5 0 00-6.364 6.364L4.757 8.25" />
                </svg>
              )}
